...
func TestRunIngestsMessage(t *testing.T) {
	...
	payload, err := json.Marshal(base.Payload{TenantID: "acme", IdempotencyKey: "k1", SubjectID: "robot:WH-17", Property: "health", Value: json.RawMessage("\"ready\""), StateKind: "observed", EventTime: time.Now().UTC(), Source: "adapter:redis", EvidenceRef: "redis/1"})
	if err != nil {
		t.Fatal(err)
	}
